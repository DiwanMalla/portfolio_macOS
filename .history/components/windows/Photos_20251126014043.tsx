  ...
  useEffect(() => {
    ...
    const fetchProjectImages = async () => {
      try {
        ...
        const projectPhotos: PhotoGalleryItem[] = [];
        ...
        data.forEach(
          (project: {
            ...
            // Handle images array (from your API)
              ...
              project.images.forEach((imageSrc, index) => {
                if (imageSrc) {
                  projectPhotos.push({
                    id: `${projectId}-${index}`,
                    src: imageSrc,
                    title: project.images!.length > 1 
                      ? `${projectTitle} (${index + 1}/${project.images!.length})`
                      : projectTitle,
                  });
                }
              });
            ...
            // Fallback to single image fields
              ...
              const imageSrc = project.image || project.thumbnail || project.cover;
              if (imageSrc) {
                projectPhotos.push({
                  id: projectId,
                  src: imageSrc,
