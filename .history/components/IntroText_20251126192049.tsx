...
export default function IntroText() {
  ...
  const nameChars = heroContent.headline.split("").map((char, i) => (
    <span
      key={i}
      className={`char inline-block ${char === " " ? "w-6 md:w-10" : ""}`}
      style={{ 
        perspective: "1000px",
  ...
  return (
    ...
    >
      ...
      <div className="overflow-hidden perspective-[1000px]">
        <h1
          ref={nameRef}
          className="text-7xl md:text-9xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-blue-200 tracking-tighter drop-shadow-2xl pb-4"
          style={{ 
      ...
      <div
        ref={lineRef}
        className="w-32 h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent rounded-full mb-6"
      />
      ...
      <div className="overflow-hidden h-10">
        ...
        >
          ...
          <span 
            ref={cursorRef}
            className={`ml-1 inline-block w-[3px] h-6 bg-blue-400 ${showCursor ? 'opacity-100' : 'opacity-0'}`}
