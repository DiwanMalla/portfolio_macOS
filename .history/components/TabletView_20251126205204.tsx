...
const TabletStatusBar = ({ light = false }: { light?: boolean }) => {
  ...
  return (
    <div className={`h-7 ${light ? 'bg-transparent' : 'bg-black/50'} backdrop-blur-xl text-white flex items-center justify-between px-4 text-xs`}>
...
const TabletIntroScreen = ({ onUnlock }: { onUnlock: () => void }) => {
  ...
  return (
    ...
    >
      {/* Ambient glow */}
      <div className="absolute w-[500px] h-[300px] bg-blue-500/20 rounded-full blur-[100px] animate-pulse" />
      
      {/* Name */}
      <h1 
        className="text-7xl md:text-8xl font-bold text-white tracking-tight text-center mb-6 relative z-10"
        style={{
          textShadow: "0 0 60px rgba(59, 130, 246, 0.5), 0 4px 20px rgba(0, 0, 0, 0.3)",
        }}
      >
      ...
      <p className="text-xl md:text-2xl text-white/90 font-medium uppercase tracking-[0.25em] text-center flex items-center">
        <span>{displayedTitle}</span>
        <span
          className={`ml-1 inline-block w-0.5 h-6 bg-blue-400 ${
      ...
      <div className="flex items-center gap-6 mt-6 text-white/60">
        ...
        <p className="text-green-400">{profile.availability}</p>
      </div>

      {/* Tap indicator */}
      <div className="absolute bottom-20 flex flex-col items-center gap-3 animate-bounce">
        <p className="text-white/50 text-sm uppercase tracking-wider">Tap anywhere to explore</p>
